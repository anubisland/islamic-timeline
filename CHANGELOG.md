...
### Fixed
- **Map zoom rebuilt around one viewBox writer (`writeMapViewBox`)** — fixes four interacting bugs from the review of the recent era work: (1) the +/−/reset controls hardcoded a 700×560 base, permanently cropping the eastern 300 units of the 1000×560 Umayyad/Abbasid maps with no way back; (2) a step whose `mapFocus` lacks `scale` skipped the viewBox write, inheriting the *previous* step's zoom (focus pulse could land outside the visible region); (3) the frame was clamped only at the top/left, showing blank gutters past the map's right/bottom edges; (4) manual zoom and per-step `scale` were two independent writers, so stepping silently discarded user zoom while the % readout went stale. Now: manual zoom multiplies the step's `scale` baseline over the active era's own `MAP_VB` base, clamps all four edges, centers when zoomed out past 1×, and resets to 100% on step/era change so the readout is always truthful. Verified in-browser on the Abbasid (1000-wide) and Meccan maps.
- **Six historical corrections in the Abbasid era** (from a content review of the new data): al-Mutawakkil *expanded* Samarra and *built* its Great Mosque (al-Mu'tasim founded the city) and honored Imam Ahmad after his ordeal (the release from the Mihna prison was under al-Mu'tasim); Zanj revolt crushed **270 AH** not 279; al-Mu'tadid's reign corrected to **279–289 AH / 892–902 CE**; al-Juwayni *headed* the Nishapur Nizamiyya and al-Ghazali *taught* at Baghdad's (neither "graduated from" it); Alp Arslan *prepared himself for martyrdom* at Manzikert (he survived it); Nishapur/Merv/Balkh fell in Genghis Khan's own campaign, not under his grandsons; al-Saffah's bay'ah dated **November 749 CE** (132 AH); step 5's "first caliph killed by a Muslim army" qualified to *Abbasid* caliph. Step 25's impossible citation (Ibn al-Athir, d. 630 AH, for the 656 AH siege) replaced with al-Suyuti's Tarikh al-Khulafa'. Sidecar re-vocalized for the changed sentences; regenerated the 60 affected clips (6 steps × 5 voices × AR+EN).
- **Browser tab title no longer goes stale** — `switchEv` rewrites the `<title>` (and its `data-ar`/`data-en`) per era but nothing restored it; after visiting any era the tab kept that era's title on the home launcher forever. `showHome()` now restores the captured document default.
- **`amb` ambience gaps closed** — `reform` (Umayyad 13–17), `noon` (22 steps), and `winter` had no CSS anywhere, rendering as a fully transparent overlay (the Umar II reform glow never appeared); added the three rules. The 7 Abbasid steps with no `amb` at all now use `day` (near-transparent, keeps the maps bright as intended).
- **Home launcher: 4 cards now lay out 2×2 on desktop** (was `repeat(3,1fr)`, orphaning the Abbasid card alone on row 2). Mobile single-column unchanged.
- **Arabic brand-title grammar completed** — non-`ال` era labels got a detached `لـ ` + space (e.g. «لـ خلافة عمر»); now an attached lām («لخلافة عمر») for all 8 affected eras.
- **Misc:** `render()` no longer defers `applyMapFocus` behind `requestAnimationFrame` (paused in hidden tabs — the documented imam-map regression pattern); `EVT` restored from localStorage is validated against `DB` (stale-cache `data.js` could otherwise crash init); duplicate GeoJSON `event_id`s (hijra_5, meccan_8, umawi_0/10/11/22×3, abassi_13/27) made unique with `_alt` suffixes; home/era/caliph/imam cards now activate with Enter/Space (they're `role="button"` divs that had no keyboard handler).
